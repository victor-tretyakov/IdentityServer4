...
public class ServerSideTicketStore : IServerSideTicketStore
{
        ...
        ILogger<ServerSideTicketStore> logger)
    {
        ...
        _store = store;
        _protector = dataProtectionProvider.CreateProtector("SessionManagement4.ServerSideTicketStore");
        _logger = logger;
    }
    ...
    public async Task RenewAsync(string key, AuthenticationTicket ticket)
    {
        ...
        var name = string.IsNullOrWhiteSpace(_options.ServerSideSessions.UserDisplayNameClaimType)
            ? null
            : ticket.Principal.FindFirst(_options.ServerSideSessions.UserDisplayNameClaimType)?.Value;
